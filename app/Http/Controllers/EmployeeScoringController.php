...
class EmployeeScoringController extends Controller
{
    /**
    ...
    {
        $currentPeriod = $request->get('period', now()->format('Y-m-01'));
        $employees = DataKaryawan::where('is_active', true)
            ->with(['penilaian' => function($query) use ($currentPeriod) {
                $query->where('periode_penilaian', $currentPeriod);
            }])
            ->get();

        $approvedCriteria = KriteriaBobot::where('status', 'Disetujui')->get();
        $availablePeriods = PenilaianKaryawan::getAvailablePeriods();

        // Calculate SAW scores and rankings
        $sawService = app(SAWCalculationService::class);
        $sawResults = $sawService->calculateSAWScores($currentPeriod);
        $sawValidation = $sawService->canPerformSAW($currentPeriod);
    ...
    public function create($employeeId, Request $request)
    {
        ...
        $period = $request->get('period', now()->format('Y-m-01'));
        ...
        // Get existing assessments for this employee and period
        ...
        return view('scoring.create', compact('employee', 'approvedCriteria', 'period', 'existingAssessments'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $period = $request->periode_penilaian;
            ...
            return redirect()->route('scoring.index', ['period' => $period])
    ...
    public function show($employeeId, Request $request)
    {
        ...
        // SAW calculations
        $sawService = app(SAWCalculationService::class);
        $sawDetails = $sawService->getEmployeeSAWDetails($employeeId, $period);
        $sawValidation = $sawService->canPerformSAW($period);
        $criteriaStats = $sawService->getCriteriaStatistics($period);

        return view('scoring.show', compact(
            'employee',
            'assessments',
            'period',
            'sawDetails',
    ...
    public function destroy($assessmentId)
    {
        ...
        $period = $assessment->periode_penilaian;
    ...
    public function getSummary(Request $request)
    {
        ...
        $assessedEmployees = PenilaianKaryawan::where('periode_penilaian', $period)
            ->distinct('id_karyawan')
            ->count('id_karyawan');
        
        $approvedCriteria = KriteriaBobot::where('status', 'Disetujui')->count();
        $totalAssessments = PenilaianKaryawan::where('periode_penilaian', $period)->count();
    ...
    public function export(Request $request)
    {
        ...
        $sawService = app(SAWCalculationService::class);
        $sawResults = $sawService->calculateSAWScores($period);
        $criteriaStats = $sawService->getCriteriaStatistics($period);
        $approvedCriteria = KriteriaBobot::where('status', 'Disetujui')->get();

        $filename = 'hasil_penilaian_' . Carbon::parse($period)->format('Y_m');
    ...
    /**
     * Bulk delete assessments for a period
     */
    public function bulkDelete(Request $request)
    {
        ...
        $query = PenilaianKaryawan::where('periode_penilaian', $request->period);
        ...
        return redirect()->route('scoring.index', ['period' => $request->period])
    ...
    public function ranking(Request $request)
    {
        ...
        return view('scoring.ranking', compact(
            'sawResults',
            'sawValidation',
            'criteriaStats',
            'approvedCriteria',
    ...
    public function getSAWDetails(Request $request)
    {
        ...
        if ($employeeId) {
            $details = $sawService->getEmployeeSAWDetails($employeeId, $period);
            return response()->json($details);
        } else {
            $allResults = $sawService->calculateSAWScores($period);
    ...
    public function getCriteriaStats(Request $request)
    {
        ...
        $sawService = app(SAWCalculationService::class);
        $stats = $sawService->getCriteriaStatistics($period);
