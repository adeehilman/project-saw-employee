@extends('inc.main')
@section('title', 'Ranking SAW - Hasil Penilaian')
@section('pages-css')
...
@section('pages-content')
    <main id="js-page-content" role="main" class="page-content">
        ...
        @if ($sawValidation['can_calculate'])
            <!-- SAW Ranking Results -->
            <x-panel.show title="Hasil Ranking SAW"
                subtitle="Peringkat karyawan berdasarkan metode Simple Additive Weighting">
                        ...
                        <tbody>
                            @foreach ($sawResults as $result)
                                <tr>
                                    <td>
                                        <span
                                            class="badge rank-badge {{ $result['rank'] <= 3 ? 'rank-' . $result['rank'] : 'badge-primary' }}">
                                            #{{ $result['rank'] }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="saw-score text-primary">
                                            {{ number_format($result['saw_score_percentage'], 2) }}</div>
                                    ...
                                    <td>
                                        <div class="criteria-breakdown">
                                            @foreach ($result['weighted_scores'] as $criteriaId => $scoreData)
                                                @php
                                                    $criteria = $approvedCriteria->firstWhere(
                                                        'id_kriteria',
                                                        $criteriaId,
                                                    );
                                                @endphp
                    ...
                    <div class="normalization-info">
                        ...
                        <ol>
                            <li><strong>Menentukan bobot untuk setiap kriteria</strong><br>
                                <small class="text-muted">Bobot ditentukan oleh Pemimpin Perusahaan saat menyetujui
                                    kriteria</small>
                            </li>
                            ...
                            <li><strong>Normalisasi Matriks</strong><br>
                                <small class="text-muted">r<sub>ij</sub> = x<sub>ij</sub> / max(x<sub>ij</sub>) untuk
                                    kriteria benefit</small>
                            </li>
...
@section('pages-script')
    ...
    <script>
        ...
        function showSAWDetails(employeeId) {
            ...
            '<div class="text-center"><i class="fal fa-spinner fa-spin"></i> Loading...</div>');
